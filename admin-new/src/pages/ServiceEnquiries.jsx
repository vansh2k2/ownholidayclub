...
const ServiceEnquiries = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/api/service-enquiries/admin');
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Failed to fetch service enquiries',
  ...
  const handleDeleteEnquiry = async (enquiry) => {
    ...
    if (result.isConfirmed) {
      try {
        await api.delete(`/api/service-enquiries/${enquiry._id}`);
        Swal.fire({
          icon: 'success',
          title: 'Deleted!',
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await api.put(`/api/service-enquiries/${id}/status`, { status: newStatus });
      Swal.fire({
        icon: 'success',
  ...
  const filteredEnquiries = enquiries.filter(enq => {
    ...
    const matchesService = serviceFilter === "all" || enq.serviceId === serviceFilter || enq.serviceName === serviceFilter;
  ...
  const exportToExcel = () => {
    const dataToExport = filteredEnquiries.map((enq, index) => ({
      ...
      "Email": enq.email,
      "Phone": enq.phone,
      "Service Name": enq.serviceName,
      "Sub Event": enq.subEvent,
      ...
      "Status": enq.status,
      "Marriage Date": enq.marriageDate ? new Date(enq.marriageDate).toLocaleDateString() : 'N/A',
      "Preferred Date": enq.checkIn ? new Date(enq.checkIn).toLocaleDateString() : 'N/A',
      "Alternative Date": enq.checkOut ? new Date(enq.checkOut).toLocaleDateString() : 'N/A',
      "Adults": enq.adults,
      "Kids": enq.kids,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Service Enquiries");
    XLSX.writeFile(workbook, "Service_Enquiries.xlsx");
  };
    ...
    <div className="w-full">
      <div className="space-y-6 bg-white p-8 border-2 border-gray-200 mt-6 rounded-none shadow-md">
        <PageHeader
          ...
          description="Manage and track service-related leads from your website"
        />
            ...
            >
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-[#C8102E] text-white">
                <div className="flex items-center gap-3">
                    <Briefcase size={18} />
                    <h3 className="text-sm font-black uppercase tracking-wider">Service Enquiry Details</h3>
                </div>
              ...
              <div className="p-8 space-y-6">
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-gray-50 p-3 border border-gray-100">
                                <label className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest block mb-1" style={{ fontFamily: "'Inter', sans-serif" }}>Preferred Date</label>
                                <p className="text-xs font-bold text-slate-700">
                                    {selectedEnquiry.checkIn ? new Date(selectedEnquiry.checkIn).toLocaleDateString() : 'N/A'}
                                </p>
                            </div>
                            <div className="bg-gray-50 p-3 border border-gray-100">
                                <label className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest block mb-1" style={{ fontFamily: "'Inter', sans-serif" }}>Alternative Date</label>
                                <p className="text-xs font-bold text-slate-700">
                ...
                <div className="pt-4 border-t border-gray-100">
                    ...
                    <div className="bg-slate-50 p-6 rounded-none border border-slate-100 italic font-medium text-slate-700 leading-relaxed shadow-inner">
                        "{selectedEnquiry.message || "No specific requests provided."}"
                    </div>
                </div>
