...
const ContactEnquiries = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/api/contact-enquiries/admin');
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Failed to fetch contact enquiries',
  ...
  const handleDeleteEnquiry = async (enquiry) => {
    ...
    if (result.isConfirmed) {
      try {
        await api.delete(`/api/contact-enquiries/${enquiry._id}`);
        Swal.fire({
          icon: 'success',
          title: 'Deleted!',
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await api.put(`/api/contact-enquiries/${id}/status`, { status: newStatus });
      Swal.fire({
        icon: 'success',
    ...
    <div className="w-full">
      <div className="space-y-6 bg-white p-8 border-2 border-gray-200 mt-6 rounded-none shadow-md">
        <PageHeader
          ...
          description="Manage and track general enquiries from your Contact Us page"
        />
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-[#C8102E] text-white">
                <div className="flex items-center gap-3">
                    <Mail size={18} />
                    <h3 className="text-sm font-black uppercase tracking-wider">Contact Enquiry Details</h3>
                </div>
                ...
                <div className="grid grid-cols-2 gap-8">
                    <div className="space-y-4">
                        ...
                        <div className="bg-blue-50 p-4 border border-blue-100">
                            <label className="text-[10px] font-semibold text-blue-400 uppercase tracking-widest block mb-1" style={{ fontFamily: "'Inter', sans-serif" }}>Subject</label>
                            <h4 className="text-md font-black text-blue-900">{selectedEnquiry.subject}</h4>
                        </div>
                    </div>
