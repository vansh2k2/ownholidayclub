...
export default function ServiceDetailModal({
  ...
  const filterOptions = [
    "All",
    ...new Set((serviceData.portfolio || []).map((port) => port.type).filter(Boolean)),
  ];
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-[2rem] shadow-2xl overflow-hidden z-10 animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col">
        ...
        {activeModal.type === "portfolio" && activeModal.data && (
          <div className="flex flex-col md:flex-row h-full overflow-y-auto">
            ...
            <div className="md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
              ...
              <p className="text-slate-600 font-sans leading-relaxed mb-8">
                Discover how we brought {activeModal.data.name} to life. From
                concept development and guest experience planning to smooth
                on-ground execution for {activeModal.data.scale.toLowerCase()},
                this project reflects our attention to detail and delivery.
              </p>
              <button
                onClick={() => {
                  setActiveModal(null);
              ...
              >
                Inquire About This Theme <ArrowRight size={16} />
              </button>

              {/* Button to go back to "All Portfolio" */}
              <button
                onClick={() => {
                  setModalFilter("All");
          ...
          <div className="flex flex-col h-full overflow-hidden bg-slate-50">
            ...
            <div className="bg-white p-8 md:p-12 border-b border-slate-100 shrink-0 relative overflow-hidden z-10 shadow-sm">
              ...
              <p className="text-slate-500 font-sans relative z-10 max-w-xl">
                Browse a curated collection of our finest{" "}
                {serviceData.title.toLowerCase()} from around the globe.
              </p>
            ...
            <div className="p-8 md:p-12 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                {serviceData.portfolio
                  .filter(
                    (port) =>
                      modalFilter === "All" ||
                      port.type.includes(modalFilter),
                  )
                  .map((port) => (
                    <div
                      key={`modal-${port.id}`}
              ...
              {serviceData.portfolio.filter(
                (port) =>
                  modalFilter === "All" || port.type.includes(modalFilter),
              ).length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-slate-500 font-sans text-sm">
                    We don't have public case studies for this category yet.
                  </p>
                  <button
