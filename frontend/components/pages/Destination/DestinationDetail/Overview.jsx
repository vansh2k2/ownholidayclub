...
export default function Overview({ destinationData }) {
  return (
            ...
            <ScrollAnimate animation="reveal-right" delay={200}>
              <div className="bg-slate-50 rounded-[2.5rem] p-8 md:p-12 border border-slate-100">
                <h3 className="text-2xl font-bold font-serif mb-8 flex items-center gap-3">
                  <Sparkles className="text-amber-500" /> Signature Highlights
                </h3>

                <div className="space-y-6 mb-10">
                  {destinationData.highlights.map((item, i) => (
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    { icon: <Waves size={16} />, label: "Private Pools" },
                    { icon: <Utensils size={16} />, label: "Gourmet Dining" },
                    { icon: <Coffee size={16} />, label: "24/7 Butler" },
                  ].map((amenity, i) => (
                    <div
                      key={i}
